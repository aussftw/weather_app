...
class App extends Component {
  ...
  getInputWeather = async e => {
    ...
    const apiСallWeather = await fetch(
      `${DEFAULT_WEATHER_URL}weather?q=${city}&units=metric&appid=${API_KEY_WEATHER}`
    )
    ...
    const apiCallForecast = await fetch(
      `${DEFAULT_WEATHER_URL}forecast?q=${city}&units=metric&appid=${API_KEY_WEATHER}`
    )
    ...
    const unsplashCall = await fetch(
      `${DEFAULT_UNSPLASH_URL}?page=1&per_page=10&query=${city}&client_id=${API_KEY_UNSPLASH}`
    )
  ...
  getCoordsWeather = async e => {
    ...
    const apiCall = await fetch(
      `${DEFAULT_WEATHER_URL}weather?lat=${lat}&lon=${long}&units=metric&appid=${API_KEY_WEATHER}`
    )
    ...
    const unsplashCall = await fetch(
      `${DEFAULT_UNSPLASH_URL}/?page=1&per_page=10&query=${
        this.state.city
      ...
      <div>
        <div>
          <Form
            ...
            coordsButton={this.getCoordsWeather}
          />
        </div>
